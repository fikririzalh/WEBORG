...
const PROFESSIONS = {
  ...
  pathfinder:{name:"Pathfinder",rank:"7",icon:"🧭",color:"#476C66",color2:"#7CB29C",skill:"Portal Step",short:"Pindahkan Pathfinder yang baru ditempatkan ke hex legal mana pun."},
  alchemist:{name:"Alchemist",rank:"8",icon:"🧪",color:"#704E91",color2:"#B882D1",skill:"Transmute",short:"Ganti 1 TOP WORKER adjacent dengan Worker acak dari tas. Worker lama kembali ke tas."}
};
const PROF_IDS = Object.keys(PROFESSIONS);
const BASE_PROF_IDS = PROF_IDS.filter(id=>!['pathfinder','alchemist'].includes(id));
...
const DIFFICULTIES = {
  ...
  arcane:{id:"arcane",name:"Mystic Expedition",icon:"🌙",bagPerProfession:3,dualCount:8,goals:4,required:4,rerolls:2,undo:"once",hardFail:true,professions:PROF_IDS}
};
...
const JOB_TEMPLATES = [
  ...
  ["Moonlit Supply Trail","line",["pathfinder","courier","alchemist"]], ["Elixir Bakery","triangle",["alchemist","baker","barista"]],
  ["Compass Calibration","line",["mechanic","pathfinder","manager"]], ["Mystic Recruitment","triangle",["recruiter","alchemist","pathfinder"]],
  ["Portal Parcel Route","line",["pathfinder","courier","courier"]], ["Arcane Team Huddle","triangle",["manager","alchemist","mechanic"]]
];
...
function cacheDom(){[
  ...
  "skillBtn","rulesBtn","bookActions","floatingBooks","floatingSkillBtn","floatingRulesBtn","resetBtn","skillDialog","rulesDialog","skillGrid","endOverlay","endEyebrow","endTitle","endText","endStats","newGameBtn"
...
function activeProfessions(diff){return diff.professions||BASE_PROF_IDS;}
function makeBag(diff){
  const ids=activeProfessions(diff),bag=[];ids.forEach(p=>{for(let i=0;i<diff.bagPerProfession;i++)bag.push(makeToken([p]));});
  const pairs=[["baker","recruiter"],["recruiter","mechanic"],["mechanic","barista"],["barista","manager"],["manager","courier"],["courier","baker"],["pathfinder","alchemist"],["alchemist","mechanic"]].filter(pair=>pair.every(id=>ids.includes(id)));
...
function createState(name,difficultyId){
  ...
  const ids=activeProfessions(diff),starters=shuffle(ids.map(p=>makeToken([p]))).slice(0,6);DIRECTIONS.map(([q,r])=>`${q},${r}`).forEach((key,i)=>board[key].push(starters[i]));
  const deck=shuffle(JOBS.filter(job=>job.jobs.every(id=>ids.includes(id))));
  return{
...
function init(){
  ...
  setupResponsiveBoard();setupFloatingBooks();
}

function setupFloatingBooks(){
  if(!els.bookActions||!els.floatingBooks)return;
  if(!('IntersectionObserver' in window)){els.floatingBooks.classList.add('show');els.floatingBooks.setAttribute('aria-hidden','false');return;}
  const observer=new IntersectionObserver(([entry])=>{
    const show=!entry.isIntersecting;
    els.floatingBooks.classList.toggle('show',show);
    els.floatingBooks.setAttribute('aria-hidden',String(!show));
  },{threshold:.25});
  observer.observe(els.bookActions);
}
...
function bind(){
  ...
  els.floatingSkillBtn.addEventListener("click",()=>els.skillDialog.showModal());els.floatingRulesBtn.addEventListener("click",()=>els.rulesDialog.showModal());
...
function beginAbility(id,origin,depth=0){
  ...
  if(id==="pathfinder")state.abilityState={type:"pathfinder",origin};
  if(id==="alchemist")state.abilityState={type:"alchemist",origin};
...
function abilityRecruiter(){
  ...
  els.workerChoices.innerHTML=options.map((t,i)=>`<button class="worker-choice-btn" data-choice="${i}"><div class="choice-token" style="background:${tokenBackground(t)}">${workerIcon(t)}</div><strong>${tokenName(t)}</strong></button>`).join("");
...
function handleAbilityHexClick(key){const a=state.abilityState;if(!a)return;if(a.type==="courier")handleCourier(key,a);if(a.type==="mechanic")handleMechanic(key,a);if(a.type==="barista")handleBarista(key,a);if(a.type==="manager")handleManager(key,a);if(a.type==="pathfinder")handlePathfinder(key,a);if(a.type==="alchemist")handleAlchemist(key,a);}
...
function handlePathfinder(key,a){
  if(key===a.origin||state.board[key].length>=MAX_STACK)return;
  const token=state.board[a.origin].pop();if(!token)return;
  state.board[key].push(token);state.placedCoord=key;showToast("Pathfinder membuka Portal Step.");finishCurrentAbility();
}
function handleAlchemist(key,a){
  if(!isAdjacent(a.origin,key)||!topToken(key))return;
  if(!state.bag.length){showToast("Tas kosong. Transmute tidak dapat dipakai.");return finishCurrentAbility();}
  const old=state.board[key].pop(),replacement=drawRandom();state.bag.push(old);state.bag=shuffle(state.bag);state.board[key].push(replacement);
  showToast(`${tokenName(old)} ditransmutasi menjadi ${tokenName(replacement)}.`);finishCurrentAbility();
}
...
function getCellInteraction(key){
  ...
  if(a.type==="pathfinder")return key!==a.origin&&state.board[key].length<MAX_STACK?"valid-target":key===a.origin?"selected-source":null;
  if(a.type==="alchemist")return isAdjacent(a.origin,key)&&topToken(key)?"valid-source":null;
  return null;
}
function renderTokenStack(stack){
  const wrap=document.createElement("div");wrap.className="token-stack";if(stack.length>=2){const x=document.createElement("div");x.className="stack-shadow one";wrap.appendChild(x);}if(stack.length>=3){const x=document.createElement("div");x.className="stack-shadow two";wrap.appendChild(x);}const t=stack[stack.length-1],disc=document.createElement("div");disc.className="cat-token";disc.style.background=tokenBackground(t);disc.innerHTML=`<span class="worker-glyph">${workerIcon(t)}</span><span class="job-icon-badge">${t.jobs.map(j=>PROFESSIONS[j].rank).join("·")}</span>`;wrap.appendChild(disc);if(stack.length>1){const c=document.createElement("span");c.className="stack-count";c.textContent=stack.length;wrap.appendChild(c);}return wrap;
}
...
function workerIcon(t){return t.jobs.map(j=>PROFESSIONS[j].icon).join("");}
