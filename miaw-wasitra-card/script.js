...
(function () {
  ...
  function startGame() {
    ...
    $("#game-role-label").textContent = state.role === "A" ? "Role A · Pembaca / Yomifuda" : "Role B · Pencari / Torifuda";
    $("#round-total").textContent = state.deck.length;
    $("#tournament-scores").hidden = state.mode !== "tournament";
    $("#score-main-label").textContent = state.mode === "practice" ? "Skor latihan" : "Kartu selesai";
    $("#score-main").textContent = "0";
    $("#team-a-score").textContent = "0";
    $("#team-b-score").textContent = "0";
    $("#session-note").innerHTML = state.mode === "practice"
  ...
  function renderReader(card) {
    const set = selectedSet();
    const pantunClass = card.teks.includes("\n") ? "is-pantun" : "";
    playSurface.innerHTML = `
      <div class="reader-layout">
          ...
          <button class="karuta-card-wrap" id="flip-card" type="button" aria-label="Balik kartu untuk melihat arti" aria-pressed="false">
            <span class="karuta-card">
              <span class="card-face front">
                <span class="card-crest" aria-hidden="true">和</span>
                <span class="card-number">${String(card.nomor).padStart(2, "0")}</span>
                <span class="card-text ${pantunClass}">${escapeHTML(card.teks)}</span>
                <span class="card-footer"><span>${escapeHTML(set.kode)}</span><span>YOMIFUDA</span></span>
              </span>
        ...
        <div class="reader-info">
          <span class="round-label">KARTU BACA · ${String(state.round + 1).padStart(2, "0")}</span>
          <h1>Bacakan kartu dengan jelas.</h1>
          <p>Sebutkan nomor, lalu baca seluruh teks. Pemain pencari mengangkat tangan ketika menemukan pasangan yang sesuai.</p>
          <div class="reader-transcript" aria-label="Teks kartu yang harus dibacakan">
            <span>TEKS YANG DIBACAKAN</span>
            <p><strong>[${card.nomor}]</strong> ${escapeHTML(card.teks)}</p>
          </div>
    ...
    const toggleFlip = () => {
      ...
      $("#flip-card-alt").textContent = pressed ? "Lihat bacaan" : "Lihat arti";
    };
    $("#flip-card").addEventListener("click", toggleFlip);
    $("#flip-card-alt").addEventListener("click", toggleFlip);
    $("#speak-card").addEventListener("click", () => speakCard(card));
    $("#reader-next").addEventListener("click", () => {
      if (state.mode === "practice") state.score += 10;
      advanceRound();
    });
  }
    ...
    playSurface.innerHTML = `
      <div class="finder-header">
        <div>
          ...
          <h1>Temukan kartu yang dibaca.</h1>
          <p>${practice ? "Target disembunyikan. Gunakan tombol dengarkan, lalu pilih kartu yang cocok." : "Dengarkan pembaca di kelompokmu. Ketuk kartu hanya untuk memberi tanda visual setelah ditemukan."}</p>
        </div>
        <div class="finder-tools">
          ${practice ? `<button class="primary-button" id="listen-target" type="button"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M11 5 6.5 9H3v6h3.5l4.5 4V5Zm4.7 3.3a5.2 5.2 0 0 1 0 7.4M18.5 5.5a9 9 0 0 1 0 13"/></svg>Dengarkan</button>` : ""}
          <button class="outline-button" id="shuffle-board" type="button">Acak susunan</button>
          ${practice ? "" : `<button class="outline-button" id="finder-next" type="button">Putaran berikutnya</button>`}
        </div>
      </div>
      <div class="board-status">
        <span><strong>${practice ? "Latihan mandiri" : "Papan turnamen"}</strong> · urutan kartu telah diacak</span>
        <span>${state.markedIds.size} ditandai</span>
      </div>
      <div class="card-board" id="card-board" aria-label="Papan kartu pencari"></div>
    `;

    renderBoard(card);
    if (practice) $("#listen-target").addEventListener("click", () => speakCard(card, false));
    $("#shuffle-board").addEventListener("click", () => {
      state.board = shuffle(state.board);
      renderBoard(card);
      showToast("Susunan kartu diacak ulang.");
    });
    if (!practice) $("#finder-next").addEventListener("click", advanceRound);
  }
  ...
  function renderBoard(target) {
    ...
    board.innerHTML = state.board.map((card) => `
      <button class="search-card ${card.teks.includes("\n") ? "is-pantun" : ""} ${state.markedIds.has(card.id) ? "is-marked" : ""}"
        type="button" data-card-id="${escapeHTML(card.id)}" aria-pressed="${state.markedIds.has(card.id)}">
        <span class="search-number"><span>${String(card.nomor).padStart(2, "0")}</span><small>TORIFUDA</small></span>
        <span class="search-text">${escapeHTML(card.teks)}</span>
    ...
    $$(".search-card", board).forEach((button) => button.addEventListener("click", () => {
      ...
      if (!selected) return;
      if (state.mode === "practice") checkAnswer(selected, target);
      else {
  ...
  function checkAnswer(selected, target) {
    ...
    $("#result-copy").textContent = correct
      ? `[${target.nomor}] ${target.teks.replaceAll("\n", " / ")}`
  ...
  function speakCard(card, includeNumber = true) {
    if (!state.sound) {
    ...
    const utterance = new SpeechSynthesisUtterance(`${intro}${card.teks.replaceAll("\n", ". ")}`);
    ...
    playSurface.innerHTML = `
      <div class="reader-stage" style="min-height:570px">
        <div class="reader-info" style="text-align:center;max-width:620px;position:relative;z-index:1">
          ...
          <h1>${state.mode === "practice" ? `${state.score} poin` : `${state.teamA} : ${state.teamB}`}</h1>
          <p>${state.mode === "practice" ? "Kamu telah menyelesaikan seluruh kartu dalam sesi ini." : `Tim ${state.teamA === state.teamB ? "A dan Tim B memperoleh skor seimbang" : state.teamA > state.teamB ? "A memimpin sesi ini" : "B memimpin sesi ini"}.`}</p>
  ...
  function scoreKey() {
    return `wasitra-best-${state.selectedSetId}-${state.role}`;
  }
