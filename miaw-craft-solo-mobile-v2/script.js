...
function renderBoard(){
  ...
  const boardViewport=els.hexBoard.parentElement;
  const viewportStyle=boardViewport?getComputedStyle(boardViewport):null;
  const horizontalPadding=viewportStyle?(parseFloat(viewportStyle.paddingLeft)||0)+(parseFloat(viewportStyle.paddingRight)||0):0;
  const verticalPadding=viewportStyle?(parseFloat(viewportStyle.paddingTop)||0)+(parseFloat(viewportStyle.paddingBottom)||0):0;
  const parentWidth=Math.max(260,(boardViewport?.clientWidth||window.innerWidth||360)-horizontalPadding);
  const parentHeight=Math.max(220,(boardViewport?.clientHeight||window.innerHeight||720)-verticalPadding);
  const mobile=window.innerWidth<=620;
  const compact=window.innerWidth<=380;
  const SQRT3=Math.sqrt(3);
  const unitCenters=coords.map(c=>({cx:SQRT3*(c.q+c.r/2),cy:1.5*c.r}));
  const unitMinX=Math.min(...unitCenters.map(c=>c.cx)),unitMaxX=Math.max(...unitCenters.map(c=>c.cx));
  const unitMinY=Math.min(...unitCenters.map(c=>c.cy)),unitMaxY=Math.max(...unitCenters.map(c=>c.cy));
  const baseWidth=(unitMaxX-unitMinX)+SQRT3;
  const baseHeight=(unitMaxY-unitMinY)+2;
  const maxWidth=isMobileDirect()?Math.min(parentWidth-12,308):Math.min(parentWidth-(mobile?10:24),mobile?(compact?326:350):430);
  const widthSide=(maxWidth-8)/baseWidth;
  const heightSide=(parentHeight-8)/baseHeight;
  const side=isMobileDirect()?Math.max(12,Math.min(28,widthSide,heightSide)):Math.max(24,Math.min(41,widthSide));
  ...
  els.hexBoard.style.setProperty('--board-w',`${boardW}px`);
  els.hexBoard.style.setProperty('--board-h',`${boardH}px`);
