...
(function () {
  ...
  // --- Mode "Cerita": menghasilkan satu kalimat kisah per kartu, dipakai
  // sebagai bahan bacaan alternatif ketika peribahasa aslinya terlalu
  // pendek (banyak yang hanya 3-6 kata) untuk dibacakan sebagai petunjuk.
  // Kalimat dibentuk dari kolom "arti" yang sudah ada, lewat kombinasi
  // templat + nama tokoh yang berputar secara deterministik, sehingga
  // setiap kartu tetap mendapat satu kalimat unik yang tidak terlalu
  // panjang maupun terlalu pendek.
  const ceritaNames = [
    "Rani", "Budi", "Siti", "Dian", "Andi", "Wulan", "Farhan", "Nadia",
    "Bima", "Sari", "Rizky", "Intan", "Doni", "Maya", "Agus", "Putri",
    "Yoga", "Lestari", "Hadi", "Ayu", "Dedi", "Nur", "Fajar", "Ratna"
  ];

  const ceritaTemplates = [
    (n, c) => `Suatu sore di kampung, ${n} akhirnya menyadari bahwa ${c}.`,
    (n, c) => `Setelah kejadian di pasar pagi itu, ${n} baru mengerti bahwa ${c}.`,
    (n, c) => `Dalam pertemuan keluarga, semua sepakat bahwa ${c}.`,
    (n, c) => `${n} bercerita kepada adiknya tentang bagaimana ${c}.`,
    (n, c) => `Guru di sekolah itu mengingatkan murid-muridnya bahwa ${c}.`,
    (n, c) => `Di tengah perjalanan pulang, ${n} merenung dan menyadari bahwa ${c}.`,
    (n, c) => `Nenek selalu berpesan kepada cucunya bahwa ${c}.`,
    (n, c) => `Kisah dari kampung seberang mengajarkan bahwa ${c}.`,
    (n, c) => `${n} baru memahami, setelah beberapa kali gagal, bahwa ${c}.`,
    (n, c) => `Sebelum mengambil keputusan penting, ${n} teringat pesan lama bahwa ${c}.`,
    (n, c) => `Teman-teman ${n} membahas panjang lebar tentang bagaimana ${c}.`,
    (n, c) => `Ayah menepuk pundak ${n} sambil berkata bahwa ${c}.`,
    (n, c) => `Cerita rakyat itu turun-temurun karena berpesan bahwa ${c}.`,
    (n, c) => `Sepulang menonton pertunjukan semalam, para penonton merenungkan bahwa ${c}.`,
    (n, c) => `Dalam obrolan di beranda rumah, tetangga ${n} menegaskan bahwa ${c}.`,
    (n, c) => `Peristiwa di kantor pagi itu membuktikan bahwa ${c}.`,
    (n, c) => `${n} menuliskan dalam buku hariannya bahwa ${c}.`,
    (n, c) => `Sang kepala desa mengingatkan warganya bahwa ${c}.`,
    (n, c) => `Ketika badai reda, para nelayan menyadari bahwa ${c}.`,
    (n, c) => `Selepas gagal panen tahun itu, ${n} memahami bahwa ${c}.`,
    (n, c) => `Di warung kopi ujung jalan, orang-orang membicarakan bagaimana ${c}.`,
    (n, c) => `${n} mengingat nasihat kakeknya bahwa ${c}.`,
    (n, c) => `Setelah bertengkar lalu berbaikan, ${n} dan sahabatnya sadar bahwa ${c}.`,
    (n, c) => `Anak-anak di kelas itu belajar dari cerita bahwa ${c}.`
  ];

  let ceritaSeed = 0;

  function ceritaClause(artiText) {
    let clause = String(artiText || "").trim();
    const semiIndex = clause.indexOf(";");
    if (semiIndex > -1) clause = clause.slice(0, semiIndex).trim();
    clause = clause.replace(/\.+$/, "").trim();
    if (!clause) return clause;
    return clause.charAt(0).toLowerCase() + clause.slice(1);
  }

  function buildCerita(artiText) {
    const clause = ceritaClause(artiText);
    const template = ceritaTemplates[ceritaSeed % ceritaTemplates.length];
    const name = ceritaNames[(ceritaSeed * 7 + 3) % ceritaNames.length];
    ceritaSeed += 1;
    return clause ? template(name, clause) : "";
  }
