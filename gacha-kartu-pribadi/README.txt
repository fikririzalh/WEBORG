RIFT ROUTE — OFFLINE CHARACTER GACHA
====================================

Cara menjalankan:
1. Ekstrak seluruh isi ZIP ke satu folder.
2. Klik dua kali file index.html.
3. Aplikasi langsung berjalan di browser tanpa internet dan tanpa instalasi.

Fitur utama:
- 112 karakter orisinal dengan rarity, atribut, statistik, skill aktif, dan skill pasif.
- Rekrut 1x dan 10x, rate-up, jaminan SR+, serta sistem pity SSR+ hingga 80 tarikan.
- Tombol tambah 1.000 atau 10.000 diamond tanpa batas.
- Premium Shop dengan lebih banyak karakter (SSR+, SSR, dan SR) yang bisa ditukar dengan Kartu Premium.
- Koleksi, pencarian, filter rarity, detail skill, riwayat, dark mode, dan light mode.
- Progres tersimpan otomatis di browser melalui localStorage.

Catatan:
- Jangan memisahkan index.html, styles.css, dan app.js.
- Untuk menghapus seluruh progres, pilih “Reset progres” pada menu samping.
- Bila penyimpanan dinonaktifkan oleh browser, aplikasi tetap dapat dimainkan tetapi progres mungkin tidak tersimpan setelah halaman ditutup.

Seluruh nama, karakter, skill, dan desain dalam demo ini bersifat orisinal.
